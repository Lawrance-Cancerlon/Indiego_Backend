...
{
    Task<List<Game>> Get(string? id = null, string? developerId = null, string? genreId = null);
    Task<Game?> Create(Game entity);
    Task<Game?> Update(string id, Game update);
...
public class GameRepository(IDatabaseService database) : IGameRepository
{
    ...
    public async Task<List<Game>> Get(string? id = null, string? developerId = null, string? genreId = null)
    {
        var filter = Builders<Game>.Filter.Empty;
        if (!string.IsNullOrEmpty(id)) filter &= Builders<Game>.Filter.Eq(x => x.Id, id);
        if (!string.IsNullOrEmpty(developerId)) filter &= Builders<Game>.Filter.Eq(x => x.UserId, developerId);
    ...
    public async Task<Game?> Update(string id, Game update)
    {
        var target = await _collection.Find(x => x.Id == id).FirstOrDefaultAsync();
        if (target == null) return null;
        return await _collection.FindOneAndReplaceAsync(x => x.Id == id, update);
    }
